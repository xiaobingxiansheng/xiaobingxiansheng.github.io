...
function projectInfoAlert() {
  ...
  href_parent.addEventListener('touchstart', function (e) {
    ...
    if (touchedElem.classList.contains('resume-minScr-href')) {
      var info = $(".resume-alert-info").
      parent().sibling(".resume-project-description").find(".resume-project-info").text();

      var alertWrap = $(".resume-alert-wrap").show().addClass(".resume-alert-wrap__active");
      $(".resume-alert-info").html(info);
    }
  })
  alerWrap.addEventListener('touchstart', function (e) {
    let touchedElem = e.target;
    if(!touchedElem.classList.contains("resume-alert-info")){
